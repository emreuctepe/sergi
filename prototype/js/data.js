/* ============================================================================
   DATA — sahte ama eksiksiz bir sayı
   ----------------------------------------------------------------------------
   Bu nesne, gerçek üründe `packages/content` derleyicisinin Markdown'dan
   üreteceği JSON'un birebir aynısıdır:  Issue → Section → Page[] → Block[]
   Yani prototipi gerçeğe bağlamak = bu dosyayı derleyici çıktısıyla
   değiştirmek. Başka hiçbir yer değişmez.
...
(function (MAG) {
  ...
  D.archive = [
    { slug: "2026-09", number: 3, title: "Kızıl Mevsim", month: "Eylül 2026", scene: "torii", current: true },
  ...
  /* ========================================================================
     TOHUM YORUMLAR
     ------------------------------------------------------------------------
     Burada değil: js/data-comments.js.
     İçerik ile yorum ayrı dosyada, çünkü gerçek üründe ayrı kaynaklardan
     geliyorlar — biri `packages/content` derleyicisinden, öteki Supabase
     `comments` tablosundan. Bu dosya derleyici çıktısıdır, yorum tutmaz.
     `D.comments` o dosyada dolduruluyor (data.js'ten hemen sonra yüklenir).
     ===================================================================== */
  ...
  D.sectionBySlug = function (slug) {
    for (var i = 0; i < D.sections.length; i++) if (D.sections[i].slug === slug) return D.sections[i];
    return null;
  };
