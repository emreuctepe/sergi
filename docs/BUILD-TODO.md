...
- [x] **1f** Derinlik: `estimateMinutes()`, mod seçici, konum koruma
      (`flow()`/`pageVisible()` 1b'de yazıldı — doğrulayıcının ihtiyacıydı).
      Konum korumanın kapanışı bir ürün hatası ortaya çıkardı: karar 1.50
- [x] **1f** Tanıtım (intro) 5 kartı — içerik `issue.ts`'te hazır, CSS
      `overlays.css`'te hazır; eksik olan bileşen ve 4 sahne
- [x] **1f** `leaves`, `waves`, `street`, `torii` sahneleri (tanıtımın çağırdıkları)
- [x] **1f** `IntroCard.scene` → `SceneName`; `string` bırakılmış tek sahne alanı
- [ ] `tokens.css`'ten kullanılmayan sayı temalarını (2026-08/10/11) ayıkla.
      ⚠️ **Ucuz değil:** `parity.test.ts` bu dosyayı prototiple bayt bayt
      bağlıyor ve korudukları arasında sayı temaları adıyla geçiyor. Silmek
      `blocks.css` için dört kez reddettiğimiz takası kabul etmek olur —
      fiyatı karar 1.52'de yazılı, karar sende
...
- [x] `portrait` sahnesi hiçbir sayfa tarafından çağrılmıyordu (tek kullanıcısı
      `sy-acilis`'ti) ve kayıtta "1f'de ya kullanıcı bulur ya kayıttan düşer"
      notuyla duruyordu. 1f'in tanıtım kartları onu çağırmadı — **kayıttan
      düştü** (karar 1.51). Sahne kadrosu 7→**6**
...
- [x] `base` sözleşmesi — karar 1.54. `SERGI_BASE=/sergi/0.1 pnpm build` çıktıyı
      `.svelte-kit/cloudflare/sergi/0.1/` altına doğru yollarla basıyor; base'siz
      derleme değişmedi. ⚠️ "0 satır" ilk tahmini yanlıştı, gerekçe 1.54'te
...
| 1.48 | Sayfada İKİNCİ balon: düzen "bir soru bir cevap" varsayımından çıktı | sy-7'ye muhabirin kısa karşılığı eklendi (`sy-7:3`, `who: 'q'`) — sayının cevaptan sonra söz alınan tek yeri. Görünürde üç satırlık bir içerik eklemesi ama 1.47'nin ızgarasında saklı bir varsayımı kırıyordu: kural `:global(.dialog--q)` diyordu, yani sayfadaki HER soru balonunu çizimle aynı ızgara gözüne koyuyordu. İkinci balon o gözde birincinin üstüne yığılırdı — üstelik ikisi de `z-index: 1` taşıdığı için sonuç "hangisi üstte" değil, okunmayan bir yığın olurdu. Kural `.page__inner > .dialog--q:first-child` olarak daraltıldı ve bu, gerçek değişmezi de yazıya döküyor: çizimle eşleşen soru sayfanın AÇILIŞ bloğudur, sonraki balonlar akışta durur. Blok kimliği sona eklendiği için (`:3`) mevcut ankrajların hiçbiri kaymadı — araya girseydi `sy-7:2`'nin çizimi `:3` olur ve ona bağlı yorumlar başka bir bloğa düşerdi. Üç sayaç birden kırmızı yandı ve üçü de bilerek oradaydı: `integrity` 98→99, `blocks.test` 98→99, `inline.test` 87→88 benzersiz metin |
...
| 1.54 | Alt yol yayını: `base` sözleşmesi — ve "0 satır" tahminimin düzeltilmesi | 0.1'i GitHub Pages'e koymanın maliyeti "klasörü kopyala, 0 satır" diye ölçülmüştü. Yanlıştı ve sebebi ÖLÇÜLEREK çıktı: derlenen HTML'deki varlık yolları MUTLAK (`/assets/…`) ve `…/sergi/0.1` altında hepsi 404 verirdi. SvelteKit kendi yollarını (`_app/`, CSS içindeki font) base verilince çeviriyor ama bunları çeviremez — onları üreten SvelteKit değil, karar 1.15'te yazılan `assetUrl`. O dosyanın kendi notu bugünü öngörmüştü: *"Bir gün alt yolda yayınlanırsa `$app/paths`'in `base`'i BURAYA girer, 90 blok içine değil."* Aynen öyle oldu, tek fonksiyon. İkinci ve daha ince sorun `paths.relative`: SvelteKit varsayılanı `true` ve o hâlde `base` sayfa başına göreli bir dizgiye dönüşüyor — kök yayın `../assets/…` üretmeye başladı. Doğru çözülür, ama YALNIZCA sondaki eğik çizgi yokken: `/sayi/2026-09/` yazan okur için `/sayi/assets/…` olur ve sayının bütün görselleri sessizce kırılır. `relative: false` ile iki yayın da mutlak yol alıyor (kök `/assets/…`, 0.1 `/sergi/0.1/assets/…`) ve kök çıktısı değişikliğimden önceki hâline döndü. `SERGI_BASE` çalışma anında DOĞRULANIYOR, cast'lenmiyor: eğik çizgisiz bir değer hatasız derlenip her varlığı 404'e düşürürdü, şimdi derlemeyi düşürüyor. 🐞 **Kendi hatam ısırık testiyle yakalandı:** `avifSrcset` içindeki `base` adlı yerelin import'u gölgelediğini ve türevleri bozduğunu sandım, teste "gerçek bir hatayı yakaladı" diye yazdım. Gölgelemeyi geri koydum, testler YEŞİL kaldı — çünkü gölgeleme o fonksiyonun kapsamında kalıyor, `assetUrl` base'i kendi modül kapsamından okuyor ve türevler hep doğruydu. Yorum düzeltildi, uydurma hata kaydı silindi. Ders 1.44/1.47/1.50'nin aynısı ama bu kez kendi teşhisime karşı: bir testin ne yakaladığı, yakaladığını SANMAKLA değil bozarak bilinir. Doğru ısırık ayrıca yapıldı — `assetUrl` base'i bırakınca 4 test kırmızı. 343→350 test |
