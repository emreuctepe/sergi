...
| **Sonraki dosya** | ⏸️ **Senin sıran:** `/dev/soylesi` — röportaj sayfası için sekiz aday, biri seçilecek (karar 1.46). Sonra 1f — `estimateMinutes()`, mod seçici, konum koruma, tanıtım kartları |
...
- [ ] 🎨 **`sy-1…sy-8`in görünümü SEÇİM BEKLİYOR** (karar 1.46). Açılış (1.45)
      beğenildi, sonraki sekiz sayfa beğenilmedi: sekizi de aynı rozet-soru-cevap-
      görsel dizilimi, sohbet dökümü gibi duruyor. Sekiz aday tasarım
