...
| — | ⚠️ | **Bu makinede kurulu Japonca yazı tipi yok** (`fc-list :lang=ja` → 0). Kanji arayüzde tofu (□) çıkıyor; palet etiketi bu yüzden romaji ("Kotoba (Söz)"), kanji yalnızca kaynak yorumlarında. `--font-ja` token'ı `tokens.css`'te tanımlı ama **hiçbir yerde kullanılmıyor** — Japonca metin bloğu geldiğinde bağlanacak. Prototip kuralı "ağ yok" olduğu için web fontu seçenek değil: gerçek Japonca metin için sisteme yerel bir mincho/gothic (ör. Noto Serif JP) kurulmalı. |
