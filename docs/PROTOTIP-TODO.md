...
- **KEŞFET VE JETON EKONOMİSİ (K1+K2) girdi.** Bir sayıyı bir modda tahmini sürenin **yarısı**
  kadar okuyan o modu tamamlamış sayılıyor. Tamamlanma anında okura hiçbir şey söylenmiyor —
  tek iz, mod seçim ekranındaki kartın **altın** zemine dönmesi. Tamamlanan her sayı bir **jeton**
  veriyor; jetonun derecesi o sayıda tamamlanan en iyi mod: `min` → açılan dergide min+mid,
  `mid` → üç mod, `full` → üç mod + yorumlarda **Doomreader prestij rozeti**.
  **Keşfet** (`js/explore.js`) 24 sahte dergiyi Pinterest kaydında gösteriyor: 3:4 kapaklar ve
  1:1 sayfa kareleri karışık, arama kutusu var. Jeton bir DERGİYE harcanıyor — kilit dergi
  bazında, o an akışta o dergiye bağlı kareler de birlikte açılıyor. Açılan dergi şimdilik
  yalnızca açılıyor (gerçek içeriği yok). **Arşiv ile karıştırma:** arşiv YEREL, bu derginin
  kendi sayıları; keşfet BAŞKA dergiler.
...
| K1 | ✅ | **Mod tamamlama ve jeton** (`js/streak.js`, yeni). Okur bir sayıyı bir modda o modun **tahmini süresinin yarısı** kadar okursa modu tamamlamış sayılıyor; tamamlanan her sayı bir **jeton** veriyor. Hedef veri olarak yazılmadı, `D.estimateMinutes(mod)`'dan geliyor — sayı büyüyünce eşik kendiliğinden büyüyor. **Bilerek sessiz:** tamamlanma anında toast/rozet/ses yok, tek görünür iz mod seçim ekranındaki kartın altın zemine dönmesi; okur oraya kendi merakıyla dönüp fark etsin diye, "şunu da yap" diyen bir görev listesi olmasın. Sayaç duvar saatini değil okuma süresini sayıyor: yalnızca sekme görünürken **ve** okur son 3 dk içinde kıpırdamışken işliyor, yani sekmeyi açık bırakıp gitmek modu tamamlatmıyor; uyuyan sekmeden gelen dev `delta` bir tura kırpılıyor. Süre `sayı:mod` çiftine yazıldığı için mod değiştirince sayaç öteki kovaya geçiyor, biriken kaybolmuyor. Yazım saniyede bir olduğundan `addModeTime` olay **yaymıyor** (yoksa her saniye tüm dinleyiciler tetiklenirdi); seyrek olan `mode:done` yayıyor. **Renk tuzağı:** altın için `--accent-2`'ye bağlanmak yanlıştı — o token sayı temasının ikinci rengi ve her sayıda altın olmak zorunda değil (2026-10'da neon pembe `#ff54a3`), bağlansaydı "tamamladım" işareti sayıdan sayıya renk değiştirirdi. Sayı temalarından bağımsız ayrı bir `--gold` tokenı eklendi. Jeton K2'de harcanıyor. Konsoldan `MAG.streak.report()` / `.forward(dk)` — açılış banner'ında bilerek duyurulmuyor. |
| K2 | ✅ | **Keşfet sayfası** (`js/explore.js` + `js/data-explore.js` + `css/explore.css`, üçü de yeni). 24 elle yazılmış sahte dergi; akış 3:4 kapaklar ve 1:1 sayfa karelerinden karışık, üstte arama (ad + slogan + etiket + çekilmiş cümle). **Arşivle karıştırılmaması gereken bir şey:** arşiv yerel, bu derginin kendi sayıları; keşfet başka dergiler — onların da kendi arşivleri var. İkisi ayrı menü satırı. **Izgara `columns` ile, grid değil:** iki en-boy oranı (3:4 ve 1:1) hiçbir satır biriminde birlikte tam oturmuyor, çok sütun akışı ise kartların doğal yüksekliğini koruyor. **Kilit dergi bazında:** bir dergiyi açan, o an akışta ona bağlı kareleri de açmış oluyor — kilit kartta değil dergide. Jeton az ve geri alınamaz harcandığı için onay **kartın içinde** soruluyor; `O.modal` tek taşıyıcı kullandığından ikinci bir modal keşfeti kapatırdı. Kilitli kart karartılmıyor, doygunluğu düşürülüyor (0.55): karartınca akış bir kilit duvarına dönüyordu, oysa keşfetin işi cezbetmek. Açık temada Lodos/Sumi gibi parlak kapaklarda künye perdesi zayıf kalıyordu — gradyan 0.88'e çıktı, üstüne metin gölgesi. |
| — | 🐞 | **İki jeton kaynağı çelişiyordu.** K1'de jeton "üç modu da bitir" kuralıyla `State.keys`'e yazılıyordu; K2 kuralı "tamamlanan her sayı bir jeton" olunca `tokens()` `modeDone`'dan türetildi ve iki kaynak ayrıştı — canlı testte `modeDone`'da `full` varken `keys` bomboştu. `keys`/`grantKey`/`keyList`/`key:earned` silindi, tek kaynak `modeDone`. **Ders:** türetilebilen bir şeyi ayrıca saklamak, kural değişince sessizce yanlışa düşüyor. |
| — | 🐞 | **Prestij rozeti otuz okurun hepsinde aynı çıkıyordu.** Dağılım `author.id` hash'inden geliyordu ama tohum okurların **`id`'si yok** — `data-comments.js` onları yalnızca `{name, emoji, color}` ile kuruyor. Hash her seferinde `"undefined"` dizgisini karıştırdığı için otuz kişi tek bir sonucu paylaşıyordu (ve o sonuç `false`'tu, yani rozet hiç görünmüyordu). Ada göre hash'leniyor: 30 okurun 4'ü rozetli, her açılışta aynı kişiler. |
...
- [x] **K1 — Mod tamamlama ve jeton.** Sürenin yarısı okununca mod tamam (sessiz),
      tek iz altın mod kartı; tamamlanan her sayı bir jeton veriyor.
- [x] **K2 — Dergi keşfet sayfası.** 24 sahte dergi, karışık 3:4/1:1 akış, arama,
      jetonla kilit açma, Doomreader prestij rozeti.
